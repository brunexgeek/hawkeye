...
// Audio buffer cache
static std::vector<OpenAudioBuffer*> __buffers;
...
OpenAudioBuffer::OpenAudioBuffer(
	const char* path,
	ALuint* buffer,
	bool streamed )
...
OpenAudioBuffer* OpenAudioBuffer::create(const char* path, bool streamed)
{
    GP_ASSERT(path);

    OpenAudioBuffer* buffer = NULL;
    if (!streamed)
    {
        unsigned int bufferCount = (unsigned int)__buffers.size();
        for (unsigned int i = 0; i < bufferCount; i++)
    ...
    if (memcmp(header, "RIFF", 4) == 0)
    {
        ...
        if (!OpenAudioBuffer::loadWav(stream.get(), alBuffer[0], streamed, streamStateWav.get()))
        {
    ...
    else if (memcmp(header, "OggS", 4) == 0)
    {
        ...
        if (!OpenAudioBuffer::loadOgg(stream.get(), alBuffer[0], streamed, streamStateOgg.get()))
        {
    ...
    buffer = new OpenAudioBuffer(path, alBuffer, streamed);
...
bool OpenAudioBuffer::loadWav(Stream* stream, ALuint buffer, bool streamed, AudioStreamStateWav* streamState)
{
    GP_ASSERT(stream);
...
bool OpenAudioBuffer::loadOgg(Stream* stream, ALuint buffer, bool streamed, AudioStreamStateOgg* streamState)
{
    GP_ASSERT(stream);
...
bool OpenAudioBuffer::streamData(ALuint buffer, bool looped)
{
